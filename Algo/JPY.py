...
class PN_values:
    try:
        def __init__(self, ctype, time_frame, sedate):
            ...
            self.pnvalues = {}
            if self.ctype == "NZDJPY" or self.ctype == "AUDJPY" or self.ctype == "USDJPY" or self.ctype == "CHFJPY" or self.ctype == "GBPJPY" or self.ctype == "EURJPY" or self.ctype == "CADJPY" :
                self.boolvalue = 100000
            else:
        ...
        def currency_main(self):
                ...
                utc_from = datetime.datetime(self.sedate.year, self.sedate.month, self.sedate.day, 00, 00, tzinfo=timezone)
                utc_to = datetime.datetime(self.sedate.year, self.sedate.month, self.sedate.day, 23, 59, tzinfo=timezone)
                    ...
                    for row_values in crates:
                        ...
                        #take positive values 
                            ...
                            if self.ctype == "NZDJPY" or self.ctype == "AUDJPY" or self.ctype == "USDJPY" or self.ctype == "CHFJPY" or self.ctype == "GBPJPY" or self.ctype == "EURJPY" or self.ctype == "CADJPY" :
                                ...
                                #print(self.ctype, zero_value, row_values[4], self.inverse)
                                #print(self.ctype, zero_value, row_values[4], (zero_value - row_values[4]), (row_values[4] * zero_value),  f'{self.inverse:.20f}')
                            ...
                            #print(timepn , self.ctype, self.pnvalues[timepn])
...
def main_loop(time_framel, sedatel):
    ...
    #currency pair list
    currency_pair_list = ["AUDJPY", "NZDJPY", "USDJPY", "CHFJPY", "GBPJPY", "EURJPY", "CADJPY"]
    while i < 7:
    ...
    jpy_fresult = OrderedDict(sorted(result.items()))
    #print(jpy_fresult)
    ...
    #creat chart
    plt.plot(list(jpy_fresult.keys()), list(jpy_fresult.values()), label = "JPY")
    plt.legend()
    plt.show()
    '''
...
'''
current_date = datetime.date(2020, 1, 6)
usd = main_loop(MT5_TIMEFRAME_H1, current_date)
...
#creat chart
plt.plot(list(usd.keys()), list(usd.values()), label = "USD")
plt.legend()
plt.show()
'''
'''
current_date = datetime.date(2020, 1, 6)
main_loop(MT5_TIMEFRAME_H1, current_date)
'''